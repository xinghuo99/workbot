...
class WorkBot:
    ...
    def _call_volcano_api(self, client, system_content, user_content, image_data_url):
        """调用火山引擎 API"""
        self.log("小华正在工作中...")
    ...
    def _call_non_volcano_api(self, client, system_content, user_content, image_data_url):
        """调用非火山引擎 API"""
    ...
    def step_work(self, user_content):
        ...
        base_url = self._api_config["base_url"]
        is_volcano = (base_url == "https://ark.cn-beijing.volces.com/api/v3")

        try:
            if is_volcano:
                result = self._call_volcano_api(client, WORKBOT_ROLE, user_content, image_data_url)
            else:
                result = self._call_non_volcano_api(client, WORKBOT_ROLE, user_content, image_data_url)
        except Exception as e:
