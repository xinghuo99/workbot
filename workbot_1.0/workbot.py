...
class WorkBot:
    ...
    def read_local_image(self, image_path=None):
        ...
        try:
            ...
            if self._api_config["base_url"] == "https://api.mindcraft.com.cn/v1/":
                return img_base64
            else:
    ...
    def move_mouse_to_coordinates(self, coordinates, solving_problem, action, type_information, scale=1):
        ...
        def _fix_coordinates(coords):
            if isinstance(coords[0], list):
                if len(coords) == 1:
                    return [_validate_coordinate(coords[0][0]), _validate_coordinate(coords[0][1])]
                else:
                    return [
                        [_validate_coordinate(coords[0][0]), _validate_coordinate(coords[0][1])],
                        [_validate_coordinate(coords[1][0]), _validate_coordinate(coords[1][1])]
                    ]
            else:
                return [_validate_coordinate(coords[0]), _validate_coordinate(coords[1])]
        ...
        if action == "drag" and isinstance(coordinates[0], list):
            ...
            if self._coordinate_callback and 0 <= start_x <= 100000 and 0 <= start_y <= 100000:
                try:
                    self._coordinate_callback((start_x, start_y))
                except Exception as e:
                    self.log(f"调用坐标回调函数时出错: {e}")

            end_x, end_y = map_coordinates(end_x, end_y, scale, img_width, img_height)
            if self._coordinate_callback and 0 <= end_x <= 100000 and 0 <= end_y <= 100000:
                try:
                    self._coordinate_callback((end_x, end_y))
                except Exception as e:
                    self.log(f"调用坐标回调函数时出错: {e}")
            ...
            if action == "click":
                ...
                self.log(f"已右键点击 ({x}, {y})")
                action_str += "已右键点击\n"
            elif action == "scroll_up":
                pyautogui.scroll(scroll_range)
                self.log(f"已向上滚动 {scroll_range}")
                action_str += f"已向上滚动 {scroll_range}\n"
            elif action == "scroll_down":
                pyautogui.scroll(-1 * scroll_range)
                self.log(f"已向下滚动 {scroll_range}")
                action_str += f"已向下滚动 {scroll_range}\n"
            else:
                self.log(f"未知操作: {action}")
        ...
        if type_information != "" and action != "hotkey":
            ...
            time.sleep(0.5)
            self.log("已发送")
            action_str += f"已发送: {type_information}\n"
    ...
    def step_work(self, user_content):
        ...
        if base_url == "https://ark.cn-beijing.volces.com/api/v3":
            self.log("小华正在工作中...")
            if self._should_exit:
                self.log("检测到退出标志，取消API调用")
                self._global_client = None
                return None

            try:
                completion = client.beta.chat.completions.parse(
                    model=self._api_config["model_name"],
                    messages=[
                    ...
                    extra_body={
                        "thinking": {
                            "type": self._ai_config["thinking_type"]
                        },
                    },
                )
            except Exception as e:
            ...
            self.log(f"非火山引擎，模型是{self._api_config['model_name']}")
            if self._should_exit:
                self.log("检测到退出标志，取消API调用")
                self._global_client = None
                return None
            ...
            try:
                ...
                if self._should_exit:
                    self.log("用户主动停止了AI执行")
                    self.fresh_display_info_vl("用户主动停止了AI执行")
                else:
                    self.log(f"API调用失败: {e}")
                    self.fresh_display_info_vl(f"API调用失败: {e}")
                self._global_client = None
        ...
        self.log(completion.choices[0].message.content)
        self._global_client = None
